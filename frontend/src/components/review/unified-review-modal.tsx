...
import { ConfidenceGauge } from "@/components/classification/confidence-gauge";
import { ContentTabPanel } from "@/components/content-review/content-tab-panel";
import type { ReviewRecord } from "@/types/review";
import type { ProductCategory } from "@/types/destinations";
import type { ContentFields, FaqItem } from "@/types/content-review";
...
const TABS: TabConfig[] = [
  { key: "payload", label: "Payload", icon: <Database className="h-3.5 w-3.5" /> },
  { key: "classification", label: "Classification", icon: <BrainCircuit className="h-3.5 w-3.5" /> },
...
interface UnifiedReviewModalProps {
  ...
  onApprove: (recordId: string, finalCategory: ProductCategory, notes: string, editedFields: ContentFields) => void;
  onEditAndApprove: (recordId: string, finalCategory: ProductCategory, notes: string, editedFields: ContentFields) => void;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl w-[95vw] h-[85vh] flex flex-col p-0">
        <ModalInner key={record.id} record={record} {...actions} />
      </DialogContent>
    </Dialog>
  );
}

// ---- Inner component (remounts per record via key) ----
interface ModalInnerProps {
  record: ReviewRecord;
  onApprove: UnifiedReviewModalProps["onApprove"];
  onEditAndApprove: UnifiedReviewModalProps["onEditAndApprove"];
  onRejectAndRegenerate: UnifiedReviewModalProps["onRejectAndRegenerate"];
  onEscalate: UnifiedReviewModalProps["onEscalate"];
}
...
function ModalInner({
  ...
  const [activeTab, setActiveTab] = useState<TabKey>("short_desc");

  // Classification form
  const [finalCategory, setFinalCategory] = useState<ProductCategory>(
    record.predictedCategory
  );
  const [notes, setNotes] = useState("");

  // Content editing
  const [editedFields, setEditedFields] = useState<ContentFields>(() =>
    cloneContentFields(record.contentFields)
  );
    ...
    <>
      ...
      <DialogHeader className="px-6 pt-6 pb-0">
        <div className="flex items-center gap-3 flex-wrap">
          <DialogTitle className="text-lg">{record.productName}</DialogTitle>
          <Badge
            variant="outline"
            className={cn(
              "text-xs capitalize",
        ...
        <DialogDescription>
          {record.destination} · {record.source} · Keyword: &quot;
      ...
      <ScrollArea className="flex-1 px-6 py-4">
        ...
        {activeTab === "payload" && (
          <div>
            <div className="mb-3">
              <h3 className="text-sm font-semibold">Normalized Payload</h3>
                    ...
                    {CATEGORY_OPTIONS.map((option) => (
                      ...
                      >
                        <input
                          ...
                          name="finalCategory"
                          value={option.value}
                          checked={finalCategory === option.value}
                          onChange={() => setFinalCategory(option.value)}
        ...
        {activeTab === "meta" && (
          <div className="space-y-4">
            <ContentTabPanel
              ...
              onChange={(v) => updateField((f) => ({ ...f, metaTitle: v }))}
              label="Meta Title"
              primaryKeyword={record.primaryKeyword}
              rows={2}
              disabled={!isReviewable}
        ...
        {activeTab === "schema" && (
          <ContentTabPanel
            ...
            onChange={(v) => updateField((f) => ({ ...f, schemaMarkup: v }))}
        ...
        <>
          ...
          <div className="px-6 py-4 flex items-center gap-2 flex-wrap">
            <Button
              ...
              onClick={() =>
                onApprove(record.id, finalCategory, notes, editedFields)
              }
            >
            ...
            <Button
              ...
              disabled={record.generationAttempt >= record.maxAttempts}
            >
      ...
      {/* Read-only footer for completed records */}
      {!isReviewable && (
        <>
          <Separator />
          <div className="px-6 py-4 flex items-center gap-3 text-sm text-muted-foreground">
            <Badge
              ...
              className={cn("text-xs", {
                ...
                "bg-red-500/10 text-red-600": record.status === "rejected",
