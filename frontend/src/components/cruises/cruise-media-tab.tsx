...
interface CruiseMediaTabProps {
  cruise: Cruise;
}

export function CruiseMediaTab({ cruise }: CruiseMediaTabProps) {
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  const firstGalleryItem = cruise.galleryJson?.[0];
  ...
  const galleryImages = (cruise.galleryJson ?? [])
    .map((item, idx) => {
      const url = typeof item === "string" ? item : item?.url;
      const alt =
        typeof item === "string"
          ? `${cruise.name} gallery ${idx + 1}`
          : item?.alt_text ?? `${cruise.name} gallery ${idx + 1}`;
